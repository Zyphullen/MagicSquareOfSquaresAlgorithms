...
public class TripletFinder : MonoBehaviour
{
    ...
    private void RunTask() // Note This script finds Only squared match 7  
    {
        ...
        long squaredCounter = 0;
        int halfCores = Math.Max(1, Environment.ProcessorCount / 2);

        while (true)
        {
            squaredCounter++;
            targetSum = squaredCounter * squaredCounter;
            ...
            foreach (var group in tripletGroups)
            {
                ...
                foreach (var diagA in triplets)
                {
                    ...
                    foreach (var rowB in triplets)
                    {
                        ...
                        grid[1] = squaredLookup[(int)rowB[0]];
                        grid[7] = squaredLookup[(int)rowB[2]];
                        grid[2] = targetSum - (grid[0] + grid[1]);
                        grid[5] = targetSum - (grid[2] + grid[8]);
                        grid[6] = targetSum - (grid[7] + grid[8]);
                        ...
                        if (totalMatches >= 6) { validGridsQueue.Enqueue((grid.ToArray(), targetSum, totalMatches)); }
                    }
                }
            }
        }
    ...
    private void ScoreCalculator(long[] grid, long targetSum, long matchCount)
    {
        ...
        {
            if (score > results[existingResultIndex].score) { results[existingResultIndex] = (targetSum, percent, grid.ToArray(), score); }            
        }
