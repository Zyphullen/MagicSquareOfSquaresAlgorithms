using System;
using System.Collections;
using System.Collections.Concurrent;
using System.Collections.Generic;
using System.Linq;
using System.Threading;
using System.Threading.Tasks;
using Unity.VisualScripting;
using UnityEngine;
using static ThreeLayerGoldMine;

public class ThreeLayerGoldMine : MonoBehaviour
{
    public long targetSum = 1;
    public long incrementer;
    public float bestScore = -1f;

    public long bestPairCount;
    public long pairTargetSum;

    public long bestGroupPairCount;
    public long bestGroupTargetSum;
 
    public long counter;

    private readonly List<long> squaredLookup = new();
    private readonly long[] squaredNums = new long[9];
    private readonly ConcurrentQueue<(long[] grid, long targetSum, long matchCount, long sqCount)> validGridsQueue = new();


    public List<PairCount> pairCountList;

    [Serializable]
    public class PairCount
    {
        public string name;
        public long pairCount;
        public long pairAmount;
    }

    public void UpdatePairCount(string name, long pairCount)
    {
        PairCount existingPair = pairCountList.Find(pc => pc.pairCount == pairCount);

        if (existingPair != null)
        {
            existingPair.pairAmount++;
            existingPair.name = name + " | PairAmount = " + existingPair.pairAmount;
        }
        else
        {
            string nameThis = name + " | PairAmount = " + 1;

            pairCountList.Add(new PairCount
            {
                name = nameThis,
                pairCount = pairCount,
                pairAmount = 1
            });

            pairCountList = pairCountList.OrderBy(pc => pc.pairCount).ToList();
        }
    }



    public List<BillionHighestCount> perBillionHighestPairCount;

    [Serializable]
    public class BillionHighestCount
    {
        public string name;
        public long HighestPairCount;
        public long TargetSum;
            
    }

    [SerializeField] private List<MagicSquareResult> magicSquares = new();
    [Serializable]
    public class MagicSquareResult
    {
        public string name;
        public long targetSum;
        public long matchCount;
        public float score;
        public float percent; // Stores closeness percentage (100 - deviation%)
        public long a, b, c, d, e, f, g, h, i;

        public MagicSquareResult(string name, long targetSum, long matchCount, float score, float percent, long a, long b, long c, long d, long e, long f, long g, long h, long i)
        {
            this.name = name;
            this.targetSum = targetSum;
            this.matchCount = matchCount;
            this.score = score;
            this.percent = percent;
            this.a = a; this.b = b; this.c = c; this.d = d; this.e = e; this.f = f; this.g = g; this.h = h; this.i = i;
        }
    }

    public void Start()
    {
        Task.Run(RunTask);
        StartCoroutine(ProcessQueue());
    }

    // old version

    // 71031708
    // 5384633868 // only perfect t / 3

    // 10304777197

    // 25,603,368,624
    // 14,593,095,075 - 112


    // ---------------------------------- / 2 since a,b - b,a
    // M - 91,576,875 - 62
    // B - 2,289,421,875 - 84
    // B - 3,080,646,075 - 110
    // B - 14,593,095,075 - 112   
    // B - 77,016,151,875 - 182
    // 
    // ---------------------------------- 1,133,258,419,383

    //private void RunTask()
    //{
    //    for (long i = 1; i <= 2500000; i += 2) squaredLookup.Add(i * i);

    //    long center, leftover, maxIndex, a, b;
    //    long maxLoop;

    //    while (true)
    //    {
    //        while (true) { if (++targetSum % 3 == 0) break; }

    //        center = targetSum / 3;
    //        leftover = targetSum - center;

    //        maxIndex = FindMaxIndex(squaredLookup, leftover);
    //        maxLoop = FindMaxIndex(squaredLookup, leftover / 2) + 1;

    //        long maxRoot = Math.Min((long)Math.Ceiling(Math.Sqrt(targetSum)), squaredLookup.Count - 1);

    //        var pairGroups = new List<(long a, long b)>();

    //        for (long i = maxIndex; i >= maxLoop; i--)
    //        {
    //            a = squaredLookup[(int)i];
    //            b = leftover - a;
    //            if (IsPerfectSquare(b))
    //            {
    //                pairGroups.Add((a, b));
    //                pairGroups.Add((b, a));
    //            }
    //        }

    //        if (pairGroups.Count < 4) continue;

    //        counter++;

    //        if (pairGroups.Count > bestPairCount) { bestPairCount = pairGroups.Count; pairTargetSum = targetSum; }

    //        foreach (var pair1 in pairGroups)
    //        {
    //            foreach (var pair2 in pairGroups)
    //            {
    //                foreach (var pair3 in pairGroups)
    //                {
    //                    foreach (var pair4 in pairGroups)
    //                    {
    //                        Interlocked.Increment(ref counter);

    //                        // 0 1 2
    //                        // 3 4 5
    //                        // 6 7 8

    //                        long[] grid = new long[9];

    //                        grid[4] = center;

    //                        grid[0] = pair1.a;
    //                        grid[8] = pair1.b;

    //                        grid[1] = pair2.a;
    //                        grid[7] = pair2.b;

    //                        grid[2] = pair3.a;
    //                        grid[6] = pair3.b;

    //                        grid[3] = pair4.a;
    //                        grid[5] = pair4.b;

    //                        if (CheckForDuplicates(grid)) continue;

    //                        long totalMatches = Sums(grid).Count(s => s == targetSum);

    //                        if (totalMatches >= 6) { validGridsQueue.Enqueue((grid, targetSum, totalMatches, 8)); }
    //                    }
    //                }
    //            }
    //        }
    //    }
    //}

    //------------------------------------------------------------------------

    private long[] numbers = new long[]
  {
        472323,
        1234875,
        2359083,
        3599427,
        6100923,
        11113875,
        11808075,
        30871875,
        32394843,
        36414867,
        54908307,
        58977075,
        59860947,
        60508875,
        79822587,
        89985675,
        100024875,
        106272675,
        122338443
  };


    // 2,846,659,248

    private void RunTask()
    {
        for (long i = 1; i <= 25000000; i++) squaredLookup.Add(i * i); // <--- could be to low bug

        long center, leftover, maxIndex, a, b;
        long maxLoop;

        long tempPairCount = 0;
        long TempTargetSum = 0;     

        long counter = 0;   
        long tempcounterz = 0;

        var TemppairGroups = new List<(long a, long b)>();

        while (true)       
        {
            while (true) { if (++targetSum % 3 == 0) break; }
           
            counter++;

            center = targetSum / 3;
            if (!IsPerfectSquare(center)) { continue; }

            leftover = targetSum - center;

            maxIndex = FindMaxIndex(squaredLookup, leftover);
            maxLoop = FindMaxIndex(squaredLookup, leftover / 2) + 1;

            long maxRoot = Math.Min((long)Math.Ceiling(Math.Sqrt(targetSum)), squaredLookup.Count - 1);

            var pairGroups = new List<(long a, long b)>();

            for (long i = maxIndex; i >= maxLoop; i--)
            {
                a = squaredLookup[(int)i];
                b = leftover - a;
                if (IsPerfectSquare(b))
                {
                    pairGroups.Add((a, b));
                }
            }

            if (pairGroups.Count < 4) continue;

           // Debug.Log("Current pairs in pairGroups: " + string.Join(", ", pairGroups.Select(p => $"({p.Item1}, {p.Item2})")));

            tempcounterz++;
         

            UpdatePairCount("Pair Count = " + pairGroups.Count, pairGroups.Count);

            if (pairGroups.Count > bestPairCount) { bestPairCount = pairGroups.Count; pairTargetSum = targetSum; }

            if (pairGroups.Count > tempPairCount)
            {
                tempPairCount = pairGroups.Count;
                TempTargetSum = targetSum;
                TemppairGroups = new();
                TemppairGroups = pairGroups;
            }




            if (counter >= 10000000000)
            {
                BillionHighestCount item = new BillionHighestCount
                {
                    name = "PairCount = " + tempPairCount + " | TargetSum = " + TempTargetSum,
                    HighestPairCount = tempPairCount,
                    TargetSum = TempTargetSum
                };
                perBillionHighestPairCount.Add(item);

                tempPairCount = 0;
                TempTargetSum = 0;
                Debug.Log("Current pairs in pairGroups: " + string.Join(", ", TemppairGroups.Select(p => $"({p.Item1}, {p.Item2})")));
                counter = 0;
            }
        }
    }

    private long CountSquares(long[] grid)
    {
        long squareCount = 9;
        for (long i = 0; i < 9; i++)
        {
            if (!IsPerfectSquare(grid[i]))
            {
                squareCount--;
                if (squareCount < 6) { break; }
            }
        }
        return squareCount;
    }

    private long FindMaxIndex(List<long> squaredLookup, long leftover)
    {
        int left = 0;
        int right = squaredLookup.Count - 1;
        int result = -1;

        while (left <= right)
        {
            int mid = left + (right - left) / 2;
            if (squaredLookup[mid] <= leftover)
            {
                result = mid;
                left = mid + 1;
            }
            else { right = mid - 1; }
        }
        return result;
    }

    private bool IsPerfectSquare(long number)
    {
        long sqrt = (long)Math.Sqrt(number);
        return sqrt * sqrt == number;
    }


    private bool CheckForDuplicates(long[] grid)
    {
        var set = new HashSet<long>();
        for (long i = 0; i < 9; i++)
        {
            if (!set.Add(grid[i])) { return true; }
        }
        return false;
    }

    private long[] Sums(long[] grid)
    {
        long[] sums = new[]
        {
        grid[0] + grid[1] + grid[2], // Row 1
        grid[3] + grid[4] + grid[5], // Row 2
        grid[6] + grid[7] + grid[8], // Row 3
        grid[0] + grid[3] + grid[6], // Col 1
        grid[1] + grid[4] + grid[7], // Col 2
        grid[2] + grid[5] + grid[8], // Col 3
        grid[0] + grid[4] + grid[8], // Diag 1
        grid[2] + grid[4] + grid[6]  // Diag 2
    };
        return sums;
    }

    private IEnumerator ProcessQueue()
    {
        int counterZ = 0;
        while (true)
        {
            while (validGridsQueue.Count > 0)
            {
                if (validGridsQueue.TryDequeue(out var result))
                {
                    ScoreCalculator(result.grid, result.targetSum, result.matchCount, result.sqCount);
                    if (++counterZ % 100 == 0) { yield return null; }

                }
            }
            yield return null;
        }
    }

    private void ScoreCalculator(long[] grid, long targetSum, long matchCount, long squareCount)
    {
        for (int i = 0; i < 9; i++) { squaredNums[i] = grid[i]; }

        long[] sums = Sums(grid);

        //float muipler = targetSum / 541875f;

        //double multiplier = (double)targetSum / 541875;

        //if (targetSum != 541875)
        //{
        //    if (multiplier == Math.Floor(multiplier))
        //    {
        //        return;
        //    }
        //}

        float percent = 100f;
        for (int i = 0; i < 8; i++)
        {
            float dev = (sums[i] > targetSum) ? (sums[i] - targetSum) * 100f / sums[i] : (targetSum - sums[i]) * 100f / targetSum;
            percent = Math.Min(percent, 100f - dev); // Convert to closeness percentage
        }

        float score = squareCount * 100 + matchCount * 100 + percent;

        string name = $"TargetSum: {targetSum} | Score: {score} | Matches: {matchCount} | Percent: {percent:F4}%";
        //$"| SqCount: {squareCount} | Muipler: {muipler:F2}";

        var newResult = new MagicSquareResult(
            name, targetSum, matchCount, score, percent,
            squaredNums[0], squaredNums[1], squaredNums[2],
            squaredNums[3], squaredNums[4], squaredNums[5],
            squaredNums[6], squaredNums[7], squaredNums[8]
        );

        var existingTarget = magicSquares.Find(m => m.targetSum == targetSum);
        if (existingTarget != null)
        {
            if (score > existingTarget.score)
            {
                magicSquares[magicSquares.IndexOf(existingTarget)] = newResult;
            }
        }
        else { magicSquares.Add(newResult); }

        magicSquares.Sort((a, b) => b.score.CompareTo(a.score));


        // New step: Remove duplicates with the highest targetSum
        //var duplicates = magicSquares
        //    .GroupBy(m => m.score)              // Group by score
        //    .Where(g => g.Count() > 1);         // Find groups with duplicates

        //foreach (var group in duplicates)
        //{
        //    // Find the entry with the highest targetSum in this group
        //    var toRemove = group.OrderByDescending(m => m.targetSum).First();
        //    magicSquares.Remove(toRemove);
        //}

        if (score > bestScore)
        {
            bestScore = score;
            Debug.Log($"High Score: {score:F4} | Matches: {matchCount} | " +
                        $"TargetSum: {targetSum} | Closeness: {percent:F2}% | \nGrid: [{string.Join(", ", squaredNums)}]");
        }


    }
}